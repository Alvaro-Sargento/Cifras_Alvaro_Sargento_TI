...
public class DigitalSignuture {
    ...
    public static void main(String[] args) {
        try {
            int[] publicKey = {3, 26};
            int[] resumo = {8, 5, 12, 12, 15};

            String resumoMensagem = "Hello";

            String[] result = encrypt(publicKey, resumoMensagem);

            System.out.println("Resumo da mensagem original: " + result[0]);
            System.out.println("Resumo decifrado: " + result[1]);
        } catch (Exception e) {
            e.printStackTrace();
            //keypair
            //keypairGenerator
